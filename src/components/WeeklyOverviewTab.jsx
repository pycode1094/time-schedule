  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
        ...
        {/* 날짜 범위 표시 */}
        <span className="text-xs text-gray-400 sm:ml-2 w-full sm:w-auto text-center sm:text-left">
            ...
            return (
              <div key={cat}>
                ...
                <div className="flex items-center gap-2 mb-1.5 sm:mb-2">
                  <span className={`text-[10px] sm:text-xs font-bold px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full ${pal.bg}`}>
                    {cat}
                  </span>
                  <span className="text-[10px] sm:text-xs text-gray-400">{items.length}개 과정</span>
                </div>

                {/* 테이블 — 모바일: 가로 스크롤 (min-width로 컬럼폭 보장) */}
                <div className="overflow-x-auto rounded-xl border border-gray-200 shadow-sm">
                  <table className="w-full border-collapse bg-white text-sm table-fixed min-w-[640px] sm:min-w-0">
                    <colgroup>
                    ...
                    <thead>
                      <tr>
                        <th className={`${pal.headerBg} ${pal.headerTxt} px-2 sm:px-3 py-2 sm:py-2.5 text-left text-[11px] sm:text-xs font-semibold border-r border-white/20`}>
                          과정
                        </th>
                        {weekDates.map((dateStr, i) => {
                          ...
                          return (
                            <th key={dateStr}
                              className={`${pal.headerBg} ${pal.headerTxt} px-1 sm:px-2 py-1.5 sm:py-2 text-center font-semibold border-r last:border-r-0 border-white/20`}>
                              <span className="text-xs sm:text-sm">{DAY_LABELS[i]}</span>
                              <span className="text-[10px] sm:text-xs font-normal opacity-80 ml-1">{Number(mm)}/{Number(dd)}</span>
                            </th>
                          );
                        })}
                      </tr>
                    ...
                    <tbody>
                      {items.map(({ course, daySummaries }) => (
                        <tr key={course.id} className="border-b border-gray-100 hover:bg-gray-50/50">
                          <td className="px-2 sm:px-3 py-1.5 sm:py-2 border-r border-gray-100 align-top">
                            <p className="text-[11px] sm:text-xs font-semibold text-gray-800 leading-snug truncate" title={course.name}>
                              {course.name}
                            </p>
                            {course.manager && (
                          ...
                          {daySummaries.map((blocks, dIdx) => (
                            <td key={dIdx}
                              className="px-1 sm:px-1.5 py-1 sm:py-1.5 border-r border-gray-100 last:border-r-0 align-top">
                              {blocks.length === 0 ? (
